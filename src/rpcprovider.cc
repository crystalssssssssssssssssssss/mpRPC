...
void RpcProvider::Run()
{
        ...
        //绑定连接回调和消息回调 ，muduo网络库的好处是分离了网络层和业务层，用户不需要关注网络io
 ...
 *                                      RpcHeader（header_str）: service_name  method_name args_size(args_size记录了参数的长度，防止Tcp粘包的问题)
 * 
 * buffer :header(4字节) + header_str + args_str
 * 注意这里的header是二进制存储(不是简单的将数字转为字符串然后存储）的占4个字节空间大小（整数不会超过4个字节）---》可以采取std::string 的 insert和copy方法
 * 
*/
...
void RpcProvider::OnMessage(const muduo::net::TcpConnectionPtr& conn, muduo::net::Buffer* buffer, muduo::Timestamp)
{
        ...
        uint32_t args_size;
        if(rpcHeader.ParsePartialFromString(rpc_header_str))
        {
                //数据反序列化成功
...
void RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr& conn, google::protobuf::Message* response)
{
        ...
        if (response->SerializeToString(&response_str)) //将响应进行序列化
        {
        ...
        conn->shutdown(); //模拟http的短链接服务， 由rpcprovider主动断开连接以免占用资源
}